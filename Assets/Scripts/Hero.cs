...
public class Hero : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!onWall() && !onTimerWall()) Physics2D.gravity = new Vector2(0f, -9.81f); // Нормальная гравитация если персонаж не висит на стене

        if (!onGround() && !onWall() && !onTimerWall()) wasInAir = true;

        if (onGround() || onWall() || onTimerWall()) doubleJump = true; // Просто возможность второго прыжка 
        ...
        {
            if (!onGround() && !onWall() && !onTimerWall()) doubleJump = false; // двойной прыжок обращается в false только если нужно прыгнуть в воздухе(рядом нет стен или земли)
            sound.Play();
            Jump();
        }
    }
    ...
    public void Jump() //Если прыжок со стены или в воздухе, flipX
    {
        if (onGround())
        {
